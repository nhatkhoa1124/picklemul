...
Java_com_picklemulmobile_Chip8Bridge_init(JNIEnv *env, jobject thiz) {
    emulator.init();
    emulator.reset();
}

JNIEXPORT void JNICALL
Java_com_picklemulmobile_Chip8Bridge_loadRom(JNIEnv *env, jobject thiz, jbyteArray rom_data) {
...
JNIEXPORT void JNICALL
Java_com_picklemulmobile_Chip8Bridge_execCycle(JNIEnv *env, jobject thiz) {
    emulator.exec();
}

JNIEXPORT void JNICALL
Java_com_picklemulmobile_Chip8Bridge_updateTimers(JNIEnv *env, jobject thiz) {
    emulator.update_timers();
}

JNIEXPORT void JNICALL
Java_com_picklemulmobile_Chip8Bridge_setKey(JNIEnv *env, jobject thiz, jint key_index, jboolean is_pressed) {
    emulator.set_key(static_cast<uint8_t>(key_index), is_pressed);
}

JNIEXPORT jbyteArray JNICALL
Java_com_picklemulmobile_Chip8Bridge_getDisplayBuffer(JNIEnv *env, jobject thiz) {
